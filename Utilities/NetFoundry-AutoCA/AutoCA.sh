...
# 20250427 - Updated to support ZT session token exchange for direct-to-controller CA create/verify.
##########################################################################################################################################
...
NF_NetworkIDs=(
  "05807e06-1292-461d-aa4d-d372edbb70ce" # 0
)
...
# Credentials file for ZT session token exchange (replaces NF_BearerToken env var).
NF_CredsFile="${HOME}/Downloads/credentials.json"
...
FLAG_CreateOnly="FALSE" # TRUE/FALSE - Flag to create identities (not enroll them) against a verified 3rd party CA.
...
#################################################################################
# Exchange MOP credentials for a ZT session token and controller URL (per network).
# Sets globals: ZT_VALUE, ZT_NWCONTROLLERURL, ZT_EXPIRESAT
FX_ExchangeToken() {
  # Expecting 1/NetworkID.
  local NF_NetworkID="${1}"
  local MyBearer MyExchangeInfo
  local CREDS_CLIENTID CREDS_PASSWORD CREDS_AUTHURL

  # Validate credentials file.
  if [[ ! -f "${NF_CredsFile}" ]]; then
    FX_AdvancedPrint "COMPLEX:M:0:1:${BRed};${Bold}" "ERROR: Credentials file not found at \"${NF_CredsFile}\"." "END"
    return 1
  fi

  CREDS_CLIENTID="$(jq -r '.clientId'        "${NF_CredsFile}")"
  CREDS_PASSWORD="$(jq -r '.password'        "${NF_CredsFile}")"
  CREDS_AUTHURL="$( jq -r '.authenticationUrl' "${NF_CredsFile}")"

  FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FGreen}" "NETWORK ID [${NF_NetworkID}]: Obtaining MOP bearer token from credentials file." "END"

  MyBearer="$(curl \
    --silent \
    --connect-timeout 10 \
    --user "${CREDS_CLIENTID}:${CREDS_PASSWORD}" \
    --request POST \
    "${CREDS_AUTHURL}" \
  ...
  if [[ -z "${MyBearer}" ]] || [[ "${MyBearer}" == "null" ]]; then
    FX_AdvancedPrint "COMPLEX:M:0:1:${BRed};${Bold}" "ERROR: Failed to obtain MOP bearer token from \"${CREDS_AUTHURL}\"." "END"
    return 1
  fi

  FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FGreen}" "NETWORK ID [${NF_NetworkID}]: Exchanging bearer token for ZT session token." "END"
  ...
  MyExchangeInfo="$(curl \
    ...
    --data '{"type":"SESSION"}' \
    "https://gateway.production.netfoundry.io/core/v3/networks/${NF_NetworkID}/exchange"
  )"

  ZT_VALUE="$(          jq -r '.value'               <<< "${MyExchangeInfo}")"
  ZT_NWCONTROLLERURL="$(jq -r '.networkControllerUrl' <<< "${MyExchangeInfo}")"
  ZT_EXPIRESAT="$(      jq -r '.expiresAt'            <<< "${MyExchangeInfo}")"

  if [[ -z "${ZT_VALUE}" ]] || [[ "${ZT_VALUE}" == "null" ]] \
  || [[ -z "${ZT_NWCONTROLLERURL}" ]] || [[ "${ZT_NWCONTROLLERURL}" == "null" ]]; then
    FX_AdvancedPrint "COMPLEX:M:0:1:${BRed};${Bold}" "ERROR: Token exchange failed for NETWORK ID [${NF_NetworkID}]. Response: ${MyExchangeInfo}" "END"
    return 1
  fi

  FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FGreen}" \
    "NETWORK ID [${NF_NetworkID}]: ZT session token obtained. Controller: [${ZT_NWCONTROLLERURL}] Expires: [${ZT_EXPIRESAT}]" "END"
...
FX_PrimaryRuntime() {
  ...
  # Input checking.
  if [[ ${FLAG_CreateOnly} == "TRUE" ]] || [[ ${FLAG_EnrollIDs} == "TRUE" ]]; then
  ...
  # Verify 3rd party CAs and/or enroll identities via direct controller using ZT session tokens.
  if [[ ${FLAG_VerifyCA} == "TRUE" ]] || [[ ${FLAG_CreateOnly} == "TRUE" ]] || [[ ${FLAG_EnrollIDs} == "TRUE" ]]; then
    if [[ ${FLAG_VerifyCA} == "TRUE" ]]; then
      ...
      for EachNetworkID in ${NF_NetworkIDs[@]}; do
        # Exchange credentials for a ZT session token scoped to this network's controller.
        if ! FX_ExchangeToken "${EachNetworkID}"; then
          FX_AdvancedPrint "COMPLEX:M:0:1:${BRed};${Bold}" "ERROR: Token exchange failed for NETWORK ID [${EachNetworkID}]. Skipping." "END"
          continue
        fi
    ...
    if [[ ${FLAG_CreateOnly} == "TRUE" ]] || [[ ${FLAG_EnrollIDs} == "TRUE" ]]; then
      FX_AdvancedPrint "COMPLEX:M:-1:1:${BBlue};${Bold}" "CREATION/ENROLLMENT OF IDENTITIES" "END"
...
#################################################################################
# Verify CAs directly against the Ziti network controller using a ZT session token.
# Relies on globals set by FX_ExchangeToken: ZT_VALUE, ZT_NWCONTROLLERURL
FX_VerifyCAs() {
  ...
  local CA_Intermediate CA_IntermediateDir NF_SelfURL NF_SelfJSON NF_SelfHref
  ...
  if [[ -f "${NF_NetworksDir}/${NF_NetworkID}/${NF_CATarget}.env" ]]; then
    ...
    FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FGreen}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: Formatting certificate for upload to controller." "END"
    ...
    # Step 1 - Controller-direct: POST /cas
    # Auth: zt-session header (obtained via FX_ExchangeToken).
    # Field names follow the Ziti Edge Management API schema (not the MOP v2 schema).
    # Response is minimal: { "data": { "id": "...", "_links": { "self": { "href": "./cas/..." } } } }
    NF_MOPJSON="$(\
      FX_PrintHelper "FILL:2:0:${Normal}" "TRUE" curl \"${ZT_NWCONTROLLERURL}/cas\" \
          -s \
          -k \
          --connect-timeout 10 \
          -X \"POST\" \
    ...
    # Parse just the ID and self link from the minimal POST response.
    NF_CAID="$(jq -r '.data.id' <<< "${NF_MOPJSON}")"
    NF_SelfHref="$(jq -r '.data._links.self.href' <<< "${NF_MOPJSON}")"

    # Step 2 - GET the self link to retrieve the full CA object including verificationToken.
    # Self hrefs are relative (./cas/<id>); resolve against ZT_NWCONTROLLERURL.
    NF_SelfURL="${ZT_NWCONTROLLERURL}/${NF_SelfHref#./}"
    FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FGreen}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: Fetching full CA details from [${NF_SelfURL}]." "END"
    NF_SelfJSON="$(\
      FX_PrintHelper "FILL:2:0:${Normal}" "TRUE" curl \"${NF_SelfURL}\" \
          -s \
          -k \
          --connect-timeout 10 \
          -X \"GET\" \
    ...
    # Parse verificationToken and the verify link href from the full CA object.
    read -d '' -r NF_CAVerifyToken NF_VerifyHref NF_NetworkJWT < <( \
      jq -r '
        .data.verificationToken,
        .data._links.verify.href,
        .data._links.jwt.href
      ' <<< "${NF_SelfJSON}"
    )
    # Resolve relative hrefs to full URLs.
    NF_VerifyURL="${ZT_NWCONTROLLERURL}/${NF_VerifyHref#./}"
    NF_JWTURL="${ZT_NWCONTROLLERURL}/${NF_NetworkJWT#./}"
    # Retrieve the enrollment JWT for FX_CreateEnrollIDs compatibility.
    NF_NetworkJWT="$(\
      FX_PrintHelper "FILL:2:0:${Normal}" "TRUE" curl \"${NF_JWTURL}\" \
          -s \
          -k \
          --connect-timeout 10 \
          -X \"GET\" \
    ...
    if [[ ${NF_CAID} != "null" ]] && [[ -n "${NF_CAID}" ]] \
    && [[ ${NF_CAVerifyToken} != "null" ]] && [[ -n "${NF_CAVerifyToken}" ]]; then
...
export NF_VerifyURL="${NF_VerifyURL}"
export NF_JWTURL="${NF_JWTURL}"
export NF_NetworkJWT="${NF_NetworkJWT}"
export NF_CAVerified="FALSE"
export ZT_NWCONTROLLERURL="${ZT_NWCONTROLLERURL}"
EOFEOFEOF
      FX_PrintHelper "FILL:2:0:${Normal}" "FALSE" cat ${NF_NetworksDir}/${NF_NetworkID}/${NF_CATarget}.env
      source "${NF_NetworksDir}/${NF_NetworkID}/${NF_CATarget}.env"
    else
      FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FRed}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: A failure occurred while communicating with the controller." "END"
      FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FRed}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: Response: ${NF_MOPJSON}" "END"
    fi
  fi
  ...
  if [[ ${NF_CAID} != "null" ]] && [[ ${NF_CAVerified} != "TRUE" ]]; then
    ...
    # Controller-direct: POST /edge/management/v1/cas/{id}/verify
    # The controller returns HTTP 200 with an updated CA object on success.
    # Capture HTTP status code separately since the body may be empty on success.
    local ZT_VerifyHTTP ZT_VerifyBody ZT_VerifyTempFile
    ZT_VerifyTempFile="$(mktemp)"

    FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FGreen}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: Submitting verification certificate to controller." "END"
    ...
    ZT_VerifyBody="$(cat "${ZT_VerifyTempFile}")"
    rm -f "${ZT_VerifyTempFile}"

    [[ ${FLAG_LearnMode} == "TRUE" ]] \
      && FX_AdvancedPrint "FILL:2:0:${Normal}" "COMPLEX:L:0:0:${Dimmed}" "VERIFY HTTP STATUS: ${ZT_VerifyHTTP}" "NEXT" \
                          "FILL:2:0:${Normal}" "COMPLEX:L:0:0:${Dimmed}" "VERIFY BODY: ${ZT_VerifyBody}" "END"

    # Treat HTTP 200 as verified; also check .data.isVerified if present in the response body.
    NF_CAVerified="FALSE"
    if [[ "${ZT_VerifyHTTP}" == "200" ]]; then
      NF_CAVerified="TRUE"
    else
      local _isVerified
      _isVerified="$(jq -r '.data.isVerified // empty' <<< "${ZT_VerifyBody}" 2>/dev/null)"
      [[ "${_isVerified}" == "true" ]] && NF_CAVerified="TRUE"
    fi
    ...
    if [[ ${NF_CAVerified} == "TRUE" ]]; then
      ...
      FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FRed}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: Verification FAILED. HTTP [${ZT_VerifyHTTP}] Body: ${ZT_VerifyBody}" "END"
...
FX_CreateEnrollIDs() {
    ...
    if [[ ! -f "${NF_NetworksDir}/${NF_NetworkID}/${NF_CATarget}.env" ]]; then
      ...
      FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FGreen}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: Network and CA information found - Loading." "END"
      source "${NF_ZIDDir}/${NF_CATarget}.env"
    fi
    if [[ -z "${NF_NetworkJWT}" ]]; then
      FX_AdvancedPrint "FILL:1:0:${Normal}" "COMPLEX:L:0:0:${FRed}" "NETWORK ID [${NF_NetworkID}]: CATARGET [${NF_CATarget}]: The network JWT was not valid or empty." "END"
      return 2
    fi

    FX_PrintHelper "FILL:2:0:${Normal}" "FALSE" cat ${NF_ZIDDir}/${NF_CATarget}.env

    if [[ -x "${NF_ZitiExec}" ]] || [[ "${FLAG_CreateOnly}" == "TRUE" ]]; then
      if [[ ${NF_CAVerified} == "TRUE" ]]; then
...
##########################################################################################################################################
# Init.
NF_CredsFileStatus="$([[ -f "${NF_CredsFile}" ]] && echo "FOUND" || echo "NOT FOUND")"
...
FX_LogoMessaging "${SystemLogo}" \
  ...
  "Credentials File [${NF_CredsFile}]: ${NF_CredsFileStatus}"

# Get options from command line.
while getopts "CVD:E:e:l" ThisOpt 2>/dev/null; do
  case ${ThisOpt} in
    "C")
      FLAG_SetupCA="TRUE"
    ;;
    ...
    *)
      FX_AdvancedPrint \
        ...
        "COMPLEX:R:10:0:${Normal}" "${MyName[0]}" "COMPLEX:L:22:0:${Normal}" " -V" "COMPLEX:L:0:0:${Normal}" "INCLUDE WORKFLOW: Per-Network, Upload/Verify/Enable INTERMEDIATE Certificate Authorities via controller. " "NEXT" \
        "COMPLEX:R:10:0:${Normal}" "${MyName[0]}" "COMPLEX:L:22:0:${Normal}" " -D [NETUUID:CANAME]" "COMPLEX:L:0:0:${Normal}" "INCLUDE WORKFLOW: Create a single identity in [NETUUID] network using [CANAME] Certificate Authority." "NEXT" \
        "COMPLEX:R:10:0:${Normal}" "${MyName[0]}" "COMPLEX:L:22:0:${Normal}" " -E [NETUUID:CANAME]" "COMPLEX:L:0:0:${Normal}" "INCLUDE WORKFLOW: Create and Enroll a single identity in [NETUUID] network using [CANAME] Certificate Authority." "NEXT" \
        "COMPLEX:R:10:0:${Normal}" "${MyName[0]}" "COMPLEX:L:22:0:${Normal}" " -e [#]" "COMPLEX:L:0:0:${Normal}" "[OPTIONAL] For option [-D] or [-E], Create/Enroll [#] quantity of identities." "NEXT" \
        ...
        "COMPLEX:R:10:0:${Normal}" "${MyName[0]}" "COMPLEX:L:22:0:${Normal}" " " "COMPLEX:L:0:0:${FYellow}" "INFO: OPTIONS [-V] and [-D/-E] require credentials file at \"${NF_CredsFile}\". (${NF_CredsFileStatus})" "NEXT" \
        "END"
      FLAG_SetupCA="FALSE"
      FLAG_VerifyCA="FALSE"
      FLAG_CreateOnly="FALSE"
...
FX_VerifyCAs() {
  ...
  local NF_IdentityAttributes="${4}"
  local NF_IdentityNamings="${5}"
  local CA_Intermediate CA_IntermediateDir NF_SelfHref NF_SelfURL NF_SelfJSON NF_JWTHref NF_JWTURL
  ...
  if [[ -f "${NF_NetworksDir}/${NF_NetworkID}/${NF_CATarget}.env" ]]; then
    ...
    # Parse verificationToken and jwt link href from the full CA object.
    NF_CAVerifyToken="$(jq -r '.data.verificationToken'   <<< "${NF_SelfJSON}")"
    NF_JWTHref="$(      jq -r '.data._links.jwt.href'     <<< "${NF_SelfJSON}")"
    NF_JWTURL="${ZT_NWCONTROLLERURL}/${NF_JWTHref#./}"
    # Retrieve the enrollment JWT for FX_CreateEnrollIDs.
    NF_NetworkJWT="$(\
      FX_PrintHelper "FILL:2:0:${Normal}" "TRUE" curl \"${NF_JWTURL}\" \
          -s \
          -k \
...
EOFEOFEOF
  ...
  if [[ ${NF_CAID} != "null" ]] && [[ ${NF_CAVerified} != "TRUE" ]]; then
    ...
    ZT_VerifyHTTP="$(curl \
      ...
      -o "${ZT_VerifyTempFile}" \
      -w "%{http_code}" \
      "${ZT_NWCONTROLLERURL}/cas/${NF_CAID}/verify"
    )"
    ZT_VerifyBody="$(cat "${ZT_VerifyTempFile}")"
    rm -f "${ZT_VerifyTempFile}"
