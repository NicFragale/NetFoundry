...
function DownloadMethod ($DLSource, $DLWhat, $DLDestination, $DLMethod="$DLDefaultMethod") {
	...
	if ($DLMethod -EQ "WEBCLIENT") {
		try {
			Invoke-WebRequest -Headers @{"Cache-Control"="no-cache"} -UseBasicParsing "$DLSource/$DLWhat" -OutFile "$DLDestination"
		} catch {
	  		$BTRETURN = $_
		}
		finally {
...
function DownloadInstall {
	...
	if ($EnrollMethod -EQ "ZCLI") {
		...
		$BTRETURN = DownloadMethod "$ZDERTarget/$ZDERVer" "$ZDERBinary" "$MyTmpPath/$ZDERBinary"
		$BTRETURN = DownloadMethod "$ZCLIRTarget/v$ZCLIRVer" "$ZCLIRZip" "$MyTmpPath/$ZCLIRZip"
...
function InitialChecking ($ParameterList=$null) {
	...
	if ((Test-Path -Path "$MyConfig" -PathType Leaf) -AND (-NOT($CentralConfURL))) {
		...
		try {
			$CentralConfContext = Invoke-WebRequest -Headers @{"Cache-Control"="no-cache"} -UseBasicParsing "$CentralConfURL"
			if ($Verbosity -GE 2) {
